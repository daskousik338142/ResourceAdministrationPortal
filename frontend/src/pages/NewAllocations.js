...
const NewAllocations = () => {
  ...
  const loadExistingData = async () => {
    try {
      console.log('NewAllocations: Starting to load existing data...');
      setIsLoading(true);
      const response = await api.getNewAllocations();

      console.log('NewAllocations: API response:', response.data);
      
      if (response.data.success && response.data.data.length > 0) {
        const records = response.data.data;
        console.log('NewAllocations: Found', records.length, 'records');
        console.log('NewAllocations: First record structure:', records[0]);
        ...
        if (records[0]._uploadInfo && records[0]._uploadInfo.headers) {
          ...
          console.log('NewAllocations: Using headers from _uploadInfo:', headerRow);
        } else {
          ...
          console.log('NewAllocations: Using headers from object keys:', headerRow);
        }
        ...
        const dataRows = records.map((record, recordIndex) => {
          const row = headerRow.map(header => {
            ...
            if (recordIndex === 0) {
              console.log(`NewAllocations: Header "${header}" -> Value "${value}"`);
            }
        ...
        console.log('NewAllocations: Transformed data rows:', dataRows.length);
        console.log('NewAllocations: Quality filtered data rows:', qualityDataRows.length);
        console.log('NewAllocations: First data row:', qualityDataRows[0]);
        ...
        console.log('NewAllocations: Data loaded successfully with', dataRows.length, 'rows and', headerRow.length, 'columns');
      } else {
        console.log('NewAllocations: No existing new allocations data found');
      ...
      console.error('NewAllocations: Error loading new allocations data:', error);
  ...
  useEffect(() => {
    console.log('NewAllocations: Component mounted, loading data...');
    loadExistingData();
  }, []);
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          // Look for "NewAllocations" tab first, then fallback to first sheet
          let sheetName = 'NewAllocations';
          if (!workbook.Sheets[sheetName]) {
            ...
            if (newAllocationsSheet) {
              sheetName = newAllocationsSheet;
            } else {
              sheetName = workbook.SheetNames[0];
              console.warn('NewAllocations sheet not found, using first sheet:', sheetName);
            }
          }
          ...
          const uploadData = async () => {
            try {
              const response = await api.uploadNewAllocations(recordsForBackend, file.name, headerRow);

              if (response.data.success) {
                console.log('New allocations data uploaded successfully to backend');
                setLastSaved(new Date().toLocaleString());
                return true;
              } else {
                console.error('Backend upload failed:', response.data.message);
                alert('Failed to save new allocations data to server: ' + response.data.message);
                return false;
              }
            } catch (error) {
          ...
          // Also check for and process AllocationReport tab if it exists
          const processAllocationReportTab = async () => {
            let allocationReportSheetName = 'AllocationReport';
            if (!workbook.Sheets[allocationReportSheetName]) {
              const sheetNames = workbook.SheetNames;
              ...
              if (allocationReportSheet) {
                allocationReportSheetName = allocationReportSheet;
              } else {
                console.log('AllocationReport sheet not found in workbook');
                return;
              }
            }

            try {
              const allocationReportWorksheet = workbook.Sheets[allocationReportSheetName];
              const allocationReportJsonData = XLSX.utils.sheet_to_json(allocationReportWorksheet, { header: 1 });

              if (allocationReportJsonData.length > 0) {
                const allocationReportHeaderRow = allocationReportJsonData[0];
                const allocationReportDataRows = allocationReportJsonData.slice(1).filter(row => 
                  row.some(cell => cell !== undefined && cell !== null && cell !== '')
                );

                if (allocationReportDataRows.length > 0) {
                  console.log('Found AllocationReport tab with', allocationReportDataRows.length, 'rows');
                  
                  const allocationReportRecords = allocationReportDataRows.map((row, index) => {
                    const record = {};
                    allocationReportHeaderRow.forEach((header, headerIndex) => {
                      record[header] = row[headerIndex] || '';
                    });
                    return record;
                  });

                  // Upload to AllocationData database
                  const allocationReportResponse = await api.uploadAllocationData(allocationReportRecords, file.name, allocationReportHeaderRow);
                  if (allocationReportResponse.data.success) {
                    console.log('AllocationReport data uploaded successfully to backend');
                  } else {
                    console.error('AllocationReport upload failed:', allocationReportResponse.data.message);
                  }
                }
              }
            } catch (error) {
              console.error('Error processing AllocationReport tab:', error);
            }
          };

          await uploadData();
          await processAllocationReportTab();
          
          // Show completion message
          const message = `File processed successfully!\n- NewAllocations data saved to New Allocations Database\n- ${workbook.Sheets['AllocationReport'] ? 'AllocationReport data saved to Allocation Database' : 'AllocationReport tab not found'}`;
          alert(message);
          
          // Save headers to backend for future use
          await api.saveNewAllocationsHeaders(headerRow);
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    return filtered;
  }, [newAllocationsData, columnFilters, searchTerm, sortConfig]);
          ...
          <div className="drop-zone-content">
            {isLoading ? (
              ...
              <>
                ...
                <p>Drag & drop your new allocations file here, or click to browse</p>
                <input
                  type="file"
                  accept=".xlsx,.xls,.csv"
