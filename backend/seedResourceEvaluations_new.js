...
const seedResourceEvaluations = async () => {
  try {
    console.log('Seeding Resource Evaluations database...');
    
    const resourceEvaluationsDB = db.getResourceEvaluationsDB();
    
    // Clear existing data
    await new Promise((resolve, reject) => {
      resourceEvaluationsDB.remove({}, { multi: true }, (err, numRemoved) => {
        if (err) reject(err);
        else {
          console.log(`Removed ${numRemoved} existing resource evaluation records`);
          resolve();
        }
      });
    });
    ...
    for (const evaluation of sampleResourceEvaluations) {
      await new Promise((resolve, reject) => {
        resourceEvaluationsDB.insert(evaluation, (err, doc) => {
          if (err) reject(err);
          else {
            console.log(`Added resource evaluation for associate: ${doc.associateName} (${doc.associateId})`);
            resolve(doc);
          }
        });
      });
    ...
    console.log('\n✅ Resource Evaluations database seeded successfully!');
    console.log(`📊 Total evaluations: ${sampleResourceEvaluations.length}`);
    ...
    console.error('❌ Error seeding resource evaluations database:', error);
    process.exit(1);
  }
};
