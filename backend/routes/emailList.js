...
// Initialize NeDB for email list
const emailListDB = new Datastore({ 
  filename: path.join(__dirname, '../data/emaillist.db'), 
  autoload: true 
});

// Ensure database is loaded
emailListDB.loadDatabase((err) => {
  if (err) {
    console.error('Error loading email list database:', err);
  } else {
    console.log('Email list database loaded successfully');
  }
});
...
router.get('/', (req, res) => {
  try {
    emailListDB.find({}, (err, docs) => {
      if (err) {
        console.error('Error fetching email list:', err);
        return res.status(500).json({
          success: false,
          message: 'Error fetching email list',
          error: err.message
        });
      }
...
router.post('/', (req, res) => {
  try {
    ...
    emailListDB.findOne({ email: email.toLowerCase() }, (err, existing) => {
      if (err) {
      ...
      emailListDB.insert(newEmail, (err, doc) => {
        if (err) {
          console.error('Error adding email:', err);
          return res.status(500).json({
            success: false,
            message: 'Error adding email address',
            error: err.message
          });
        }

        console.log('Email added successfully:', doc.email);
        res.status(201).json({
          success: true,
          data: doc,
...
router.put('/:id', (req, res) => {
  try {
    ...
    emailListDB.findOne({ 
      ...
      emailListDB.update({ _id: id }, { $set: updateData }, {}, (err, numReplaced) => {
        ...
        emailListDB.findOne({ _id: id }, (err, doc) => {
          if (err) {
            console.error('Error fetching updated email:', err);
            return res.status(500).json({
              success: false,
    ...
    console.error('Error in PUT /email-list:', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: error.message
...
router.get('/active', (req, res) => {
  try {
    ...
    console.error('Error in GET /email-list/active:', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: error.message
...
router.post('/send-summary', async (req, res) => {
  try {
    ...
    console.error('Error in POST /email-list/send-summary:', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: error.message
